...
model = Sequential([
    ...
    Dropout(0.5),
    Dense(5, activation='softmax')  # 5 classes with softmax activation
])
...
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues',
           xticklabels=['Curly', 'Dreadlocks', 'Straight', 'Wavy', 'Kinky'],
           yticklabels=['Curly', 'Dreadlocks', 'Straight', 'Wavy', 'Kinky'])
plt.ylabel('Actual')
...
def predict_hairtype(image_path, model):
...
prediction = predict_hairtype(image_path, model)
